################################################################################
#                    Test last round key of DPA result                         #
################################################################################
#                                                                              #
# Sichere Implementierung kryptographischer Verfahren WS 2015-2016             #
#                                                                              #
# Technische Universitaet Muenchen                                             #
# Institute for Security in Information Technology                             #
# Prof. Dr.-Ing. Georg Sigl                                                    #
#                                                                              #
################################################################################

############################## IMPORT MODULES ##################################
import numpy as np
from Crypto.Cipher import AES

############################## Set constants ###################################
SBox = np.array([
    ,
], np.uint8)

Rcon = np.array([
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40,
    0x80, 0x1B, 0x36, 0x6C, 0xD8, 0xAB, 0x4D, 0x9A,
    0x2F, 0x5E, 0xBC, 0x63, 0xC6, 0x97, 0x35, 0x6A,
    0xD4, 0xB3, 0x7D, 0xFA, 0xEF, 0xC5, 0x91, 0x39,
], np.uint8)

################################ Test key ######################################
def reverse_key_exp(last_round_key):
    """Reverse key expansion to retrieve original key from last round key"""

    #inverted key expansion to derive original key
    k = np.zeros((11,16), np.uint8)
    k[10] = np.array(last_round_key, dtype = np.uint8)
    for i in xrange(10,0,-1):
        k[i-1,4:16]=k[i,0:12]^k[i,4:16]
        k[i-1,0:4]=k[i,0:4]^SBox[k[i-1,(13,14,15,12)]]^[Rcon[i],0,0,0]
    secret_key = k[0]

    return secret_key


def test_key(last_round_key, plaintext, ciphertext):
    """Tests last round key with given encryption pair."""

    #inverted key expansion to derive original key
    secret_key = reverse_key_exp(last_round_key)

    #perform AES encryption with derived key
    aes = AES.new(secret_key,AES.MODE_ECB)
    calculated_cipher = np.array(bytearray( aes.encrypt(plaintext) ),  dtype = np.uint8)

    #check result with data
    if (calculated_cipher == ciphertext).all():
        return True
    else:
        return False

if __name__ == "__main__":
    """Tests the key saved in key.txt"""
    import csv              # Add support for comma separated value (CSV) files
    import load_traces      # Loads traces

    #open input file (path defined in load_traces)
    input_data = load_traces.load_traces()

    # Load ciphertexts into a matrix: trace-number x 16 bytes ciphertext
    ciphertexts = input_data.get_ciphertexts()
    # Load plaintexts into a matrix: trace-number x 16 bytes plaintext
    plaintexts = input_data.get_plaintexts()

    #read key from key file
    with open( "./key.txt", "rb" ) as keyF:
        reader = csv.reader(keyF)
        last_round_key_hex = reader.next()
    last_round_key = [int(b,16) for b in last_round_key_hex]

    #test key
    if test_key(last_round_key, plaintexts[0], ciphertexts[0]):
        print "Congratulations! Your key is right."
    else:
        print "Your key is wrong :-("
